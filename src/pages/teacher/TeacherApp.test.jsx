...
vi.mock("../../api/teacher.js", () => ({
  teacherApi: {
    ...
    subjects: vi.fn(async () => ({ items: [{ id: "soil-mechanics", title: "土力学", status: "active", knowledgePointCount: 4, questionCount: 12 }], total: 1 })),
    knowledgePoints: vi.fn(async () => ({ items: [
      { id: "soil-darcy", subjectId: "soil-mechanics", chapter: "第二章 土的渗透性", name: "达西定律", questionCount: 6, status: "active", editable: false },
      { id: "soil-permeability", subjectId: "soil-mechanics", chapter: "第二章 土的渗透性", name: "土的渗透性", questionCount: 8, status: "active", editable: false },
    ], total: 2, statusCounts: { active: 2 } })),
    ...
    uploadResource: vi.fn(), createKnowledgePoint: vi.fn(), updateKnowledgePoint: vi.fn(), deleteKnowledgePoint: vi.fn(), mergeKnowledgePoint: vi.fn(), createQuestion: vi.fn(), updateQuestion: vi.fn(), deleteQuestion: vi.fn(), copyQuestion: vi.fn(), createAssignment: vi.fn(), gradeSubmission: vi.fn(), createNotice: vi.fn(),
...
test("题库批量导入显示预检错误并阻止入库", async () => {
  ...
  expect(screen.getByText("共享 DOCX 题库由服务器统一入库")).toBeInTheDocument();
  expect(screen.getByText("无需重复上传原始文档")).toBeInTheDocument();
...
test("干净的题库预检结果可以确认入库", async () => {
  ...
  await waitFor(() => expect(teacherApi.questions).toHaveBeenCalledTimes(2));
});
...
test("编辑多项选择题时以 JSON 数组保留所有正确选项", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "编辑" }));
  expect(screen.getByLabelText("A 为正确答案")).toBeChecked();
  expect(screen.getByLabelText("B 为正确答案")).not.toBeChecked();
  expect(screen.getByLabelText("C 为正确答案")).toBeChecked();
  fireEvent.click(screen.getByRole("button", { name: "保存题目" }));
...
test("编辑填空题时以 JSON 同义答案列表保留生产载荷", async () => {
  ...
  await screen.findByText(question.text);
  fireEvent.click(screen.getByRole("button", { name: "编辑" }));
  expect(screen.getByLabelText("可接受答案")).toHaveValue("太沙基\nTerzaghi");
  fireEvent.click(screen.getByRole("button", { name: "保存题目" }));
  await waitFor(() => expect(teacherApi.updateQuestion).toHaveBeenCalledWith(question.id, {
...
test("多项选择题没有正确选项时在前端提示且不提交", async () => {
  const question = canonicalQuestion({ id: "missing-multiple-answer", questionType: "多项选择题", correctAnswer: ["A"] });
  teacherApi.questions.mockResolvedValueOnce({ items: [question], total: 1 });
  renderTeacher(vi.fn(), "/teacher/question-bank");
  await screen.findByText(question.text);
  fireEvent.click(screen.getByRole("button", { name: "编辑" }));
  ...
  fireEvent.click(screen.getByRole("button", { name: "保存题目" }));
  expect(await screen.findByText("多项选择题至少选择一个正确答案")).toBeInTheDocument();
  expect(teacherApi.updateQuestion).not.toHaveBeenCalled();
});


test("填空题没有可接受答案时在前端提示且不提交", async () => {
  const question = canonicalQuestion({ id: "missing-fill-answer", questionType: "填空题", options: [], correctAnswer: ["太沙基"] });
  teacherApi.questions.mockResolvedValueOnce({ items: [question], total: 1 });
  renderTeacher(vi.fn(), "/teacher/question-bank");
  await screen.findByText(question.text);
  fireEvent.click(screen.getByRole("button", { name: "编辑" }));
  fireEvent.change(screen.getByLabelText("可接受答案"), { target: { value: "  " } });
  fireEvent.click(screen.getByRole("button", { name: "保存题目" }));
  expect(await screen.findByText("填空题至少需要一个可接受答案")).toBeInTheDocument();
  expect(teacherApi.updateQuestion).not.toHaveBeenCalled();
});
