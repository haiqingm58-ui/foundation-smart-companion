...
export default function TeacherApp() {
  ...
  return (
    <PortalShell roleLabel="教师工作台" navItems={navItems} active={active} onNavigate={go} user={user} onLogout={logout}>
      ...
      {modal?.type === "question" && <QuestionModal close={() => setModal(null)} done={() => { setModal(null); refresh(); notify("题目已保存"); }} />}
...
function ResourcesView({ data, loading, error, retry, open, notify, refresh }) {
  const remove = async (item) => {
    if (!window.confirm(`确认删除“${item.title}”吗？相关 RAG 切块也会删除。`)) return;
    try { await teacherApi.deleteResource(item.id); notify("资料已删除"); refresh(); } catch (reason) { notify(reason.message, "error"); }
  };
...
function QuestionsView({ data, loading, error, retry, open, notify, refresh }) {
  ...
  const remove = async (item) => { if (!window.confirm("确认删除这道自建题目吗？")) return; try { await teacherApi.deleteQuestion(item.id); notify("题目已删除"); refresh(); } catch (reason) { notify(reason.message, "error"); } };
  return <ViewState loading={loading} error={error} retry={retry}><PageHeading eyebrow="教学内容" title="题库管理" description="教材题目只读，自建题目可编辑和用于作业。" actions={<button className="portalPrimary" onClick={() => open({ type: "question" })}><Plus size={17} />新建题目</button>} /><Panel title="课程题库" description={`共 ${data?.total || 0} 道题`} actions={<SearchField value={query} onChange={setQuery} placeholder="搜索题干或知识点" />}><DataTable rows={rows} columns={[{ key: "text", label: "题目", render: (row) => <div className="portalQuestionCell"><strong>{row.text}</strong><span>{row.chapter || "未关联章节"} · {row.knowledgePoint || "未标注知识点"}</span></div> }, { key: "questionType", label: "题型" }, { key: "difficulty", label: "难度" }, { key: "points", label: "分值" }, { key: "source", label: "来源", render: (row) => row.source === "textbook" ? "教材题库" : "教师自建" }, { key: "actions", label: "操作", render: (row) => row.source === "teacher" ? <button className="portalIconDanger" onClick={() => remove(row)} aria-label={`删除题目 ${row.text}`}><Trash2 size={16} /></button> : "只读" }]} empty="题库中暂无题目" /></Panel></ViewState>;
}


function AssignmentsView({ data, loading, error, retry, open }) {
  return <ViewState loading={loading} error={error} retry={retry}><PageHeading eyebrow="作业流程" title="作业管理" description="从题库选题并发送给强绑定范围内的学生。" actions={<button className="portalPrimary" onClick={() => open({ type: "assignment" })}><Plus size={17} />创建作业</button>} /><Panel title="作业列表" description={`共 ${data?.total || 0} 份作业`}><DataTable rows={data?.items} columns={[{ key: "title", label: "作业名称", render: (row) => <div className="portalTitleCell"><strong>{row.title}</strong><span>{row.description || "无说明"}</span></div> }, { key: "dueAt", label: "截止时间", render: (row) => formatDate(row.dueAt) }, { key: "totalPoints", label: "总分" }, { key: "status", label: "状态", render: (row) => <StatusBadge status={row.status} /> }]} empty="尚未创建作业" /></Panel></ViewState>;
}
...
function ResourceModal({ close, done }) {
  ...
  const submit = async (event) => { event.preventDefault(); setBusy(true); setError(""); try { await teacherApi.uploadResource(new FormData(event.currentTarget)); done(); } catch (reason) { setError(reason.message); setBusy(false); } };
  return <Modal title="上传教学资料" onClose={close}><form className="portalForm" onSubmit={submit}><Field label="选择文件" hint="支持 PDF、Word、PPT、XLSX、Markdown、TXT 与图片，服务器会安全提取文本。"><input type="file" name="file" required /></Field><div className="portalFormGrid"><Field label="关联章节"><input name="chapter" placeholder="例如：第3章 桩基础" /></Field><Field label="知识点"><input name="knowledgePoint" placeholder="例如：单桩竖向承载力" /></Field></div><Field label="可见范围"><select name="visibility" defaultValue="class"><option value="class">绑定班级可见</option><option value="private">仅自己可见</option></select></Field>{error && <p className="portalFormError">{error}</p>}<div className="portalFormActions"><button type="button" onClick={close}>取消</button><button className="portalPrimary" disabled={busy}>{busy ? "正在处理..." : "上传并建立索引"}</button></div></form></Modal>;
}


function QuestionModal({ close, done }) {
  const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  const submit = async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); setBusy(true); try { await teacherApi.createQuestion({ text: form.get("text"), questionType: form.get("questionType"), difficulty: form.get("difficulty"), points: Number(form.get("points")), chapter: form.get("chapter"), knowledgePoint: form.get("knowledgePoint"), correctAnswer: form.get("correctAnswer"), explanation: form.get("explanation"), options: [], rubric: [] }); done(); } catch (reason) { setError(reason.message); setBusy(false); } };
  return <Modal title="新建题目" onClose={close} wide><form className="portalForm" onSubmit={submit}><Field label="题干"><textarea name="text" rows="4" required /></Field><div className="portalFormGrid"><Field label="题型"><select name="questionType"><option>简答题</option><option>单项选择题</option><option>多项选择题</option><option>判断题</option><option>填空题</option><option>计算题</option></select></Field><Field label="难度"><select name="difficulty"><option>基础</option><option>中等</option><option>提高</option></select></Field><Field label="分值"><input name="points" type="number" min="1" defaultValue="10" required /></Field><Field label="章节"><input name="chapter" placeholder="第3章 桩基础" /></Field></div><Field label="知识点"><input name="knowledgePoint" /></Field><Field label="标准答案"><textarea name="correctAnswer" rows="3" /></Field><Field label="答案解析"><textarea name="explanation" rows="3" /></Field>{error && <p className="portalFormError">{error}</p>}<div className="portalFormActions"><button type="button" onClick={close}>取消</button><button className="portalPrimary" disabled={busy}>保存题目</button></div></form></Modal>;
}
