...
function PasswordModal({ item, close, done }) {
  ...
  return <Modal title={`重置密码：${item.name}`} onClose={close}><form className="portalForm" onSubmit={submit}><PasswordField className="portalField" label="新密码" hint="至少 8 位，必须包含大写字母、小写字母和数字。" name="password" minLength="8" autoComplete="new-password" required /><p className="portalFormNote">保存后该账号的全部现有会话将失效，并在下次登录时要求修改密码。</p>{error && <p className="portalFormError">{error}</p>}<div className="portalFormActions"><button type="button" onClick={close}>取消</button><button className="portalPrimary" disabled={busy}><KeyRound size={16} />确认重置</button></div></form></Modal>;
}
